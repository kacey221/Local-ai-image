...
test('maps gpt-image-2-vip 4K square output to a 4096 request size', () => {
  assert.equal(
    mapAspectRatioToRightCodesSize('1:1', '4K', 'gpt-image-2-vip'),
    '4096x4096',
  );
});

test('coerces unsupported gpt-image-2 4K requests down to 1K size', () => {
  assert.equal(
    mapAspectRatioToRightCodesSize('1:1', '4K', 'gpt-image-2'),
    '1024x1024',
  );
});

test('maps vip 4K portrait output to the larger portrait request size', () => {
  assert.equal(
    mapAspectRatioToRightCodesSize('3:4', '4K', 'gpt-image-2-vip'),
    '4096x6144',
  );
});
...
test('uses the explicitly selected Right Codes model when provided', () => {
  ...
  assert.equal(request.size, '4096x4096');
});
...
test('builds a Right Codes upscale request that preserves the source image composition', () => {
  ...
  assert.equal(request.model, 'nano-banana-pro');
  assert.equal(request.size, '6144x4096');
