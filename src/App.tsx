  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-slate-50 text-slate-800 overflow-hidden font-sans antialiased select-none relative">
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
        <div
          className={`absolute top-[-10%] left-[-10%] w-[600px] h-[600px] rounded-full bg-pink-200/40 ${
            isCanvasInteracting ? 'blur-[110px] opacity-70' : 'blur-[130px] animate-pulse'
          }`}
          style={isCanvasInteracting ? undefined : { animationDuration: '10s' }}
        />
        <div
          className={`absolute bottom-[-10%] right-[-10%] w-[700px] h-[700px] rounded-full bg-sky-200/35 ${
            isCanvasInteracting ? 'blur-[120px] opacity-70' : 'blur-[150px] animate-pulse'
          }`}
          style={isCanvasInteracting ? undefined : { animationDuration: '12s' }}
        />
        <div
          className={`absolute top-[30%] left-[20%] w-[500px] h-[500px] rounded-full bg-fuchsia-200/30 ${
            isCanvasInteracting ? 'blur-[95px] opacity-65' : 'blur-[120px] animate-pulse'
          }`}
          style={isCanvasInteracting ? undefined : { animationDuration: '14s' }}
        />
        <div
          className={`absolute bottom-[20%] left-[-10%] w-[450px] h-[450px] rounded-full bg-rose-100/35 ${
            isCanvasInteracting ? 'blur-[80px] opacity-60' : 'blur-[100px] animate-pulse'
          }`}
          style={isCanvasInteracting ? undefined : { animationDuration: '15s' }}
        />
      </div>

      <header
        className={`h-14 border-b border-slate-200/70 flex items-center justify-between px-4 shrink-0 z-30 shadow-xs relative ${
          isCanvasInteracting ? 'bg-white/92' : 'bg-white/75 backdrop-blur-xl'
        }`}
      >
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span className={`absolute top-1 right-1 w-1.5 h-1.5 bg-red-500 rounded-full ${isCanvasInteracting ? '' : 'animate-ping'}`} />
          </button>
          ...
          >
            ...
            <span className={`absolute bottom-0 right-0 w-2 h-2 bg-emerald-500 border border-white rounded-full ${isCanvasInteracting ? '' : 'animate-pulse'}`} />
          </div>
        </div>
      </header>
